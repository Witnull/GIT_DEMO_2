...
function updCells() {
  cellArray.forEach(() => {
    ...
    if (gameMode === 3 || gameMode === 6) {
      if (ran >= 40 && ran <= 50) {
        if (ran >= 45 && ran <= 46) spawnApple(99);
        else spawnApple();
      }
    }
...
function initGame() {
  ...
  if (gameMode === 4 || gameMode === 6) {
    meteor_timerId = setInterval(
      spawnMeteor,
      default_meteorSpawn / diff_set[diff].speed
    );
  }
  ...
  timerId_autoMove = setInterval(
    autoMove,
    speedMode_set[speedMode] / diff_set[diff].speed
  );
  DisplayStuff();
}
initGame();
...
wallModeBtn.addEventListener("click", () => {
  wallMode++;
  if (wallMode > max_WallMode - 1) wallMode = 0;
  initGame();
});
gameModeBtn.addEventListener("click", () => {
  gameMode++;
  if (gameMode > max_gameMode - 1) gameMode = 0;
  initGame();
});
snakeModeBtn.addEventListener("click", () => {
  snakeMode++;
  if (snakeMode > max_snakeMode - 1) snakeMode = 0;
  initGame();
});
diffModeBtn.addEventListener("click", () => {
  diff++;
  if (diff > max_diffMode - 1) diff = 0;
  initGame();
});
speedModeBtn.addEventListener("click", () => {
  speedMode++;
  if (speedMode > speedMode_set.length - 1) speedMode = 0;
  initGame();
});

colorModeBtn.addEventListener("click", () => {
  colorMode++;
  if (colorMode > max_colorMode - 1) colorMode = 0;
  //initGame()
  DisplayStuff();
});
...
function autoMove() {
  ...
  if (currDirection != 0) {
    ...
    if (currDirection === 1 && currPlrIdx < 0) currPlrIdx += cellArray.length;
    if (currDirection === 2 && currPlrIdx > cellArray.length)
      currPlrIdx -= cellArray.length;
    if (
      currDirection === 3 &&
      currPlrIdx % diff_set[diff].mapSize === diff_set[diff].mapSize - 1
    )
      currPlrIdx += diff_set[diff].mapSize;
    ...
    if (cellArray[currPlrIdx].classList.contains(appleTag)) {
      ...
      if ((gameMode === 1 || gameMode === 4) && score % 10 == 0) spawnApple(99);

      let ran = Math.random() * 100;
      if (gameMode >= 2 && ran >= 38 && ran <= 48) spawnApple(99);
    }
    ...
    if (
      cellArray[currPlrIdx].classList.contains(wallTag) ||
      cellArray[currPlrIdx].classList.contains(bodyTag)
    ) {
      YouLose();
    }
    ...
    if (tailArr.size() > snakeLength) {
      ...
      clist.forEach((clsi) => {
        if (clsi.slice(0, 5) == "color") clist.remove(clsi);
      });
    }
...
function spawnApple(code = 0) {
  ...
  } else if (code === 8038) cellArray[AplIdx].classList.add(bombTag);
  else cellArray[AplIdx].classList.add(appleTag);
}
function spawnMeteor() {
  ...
  if (
    ...
    clist.contains(bodyTag) ||
    clist.contains(appleTag) ||
    clist.contains(meteorTag)
  )
    spawnMeteor();
  /// remove class w expection
  clist.forEach((clsi) => {
    if (clsi == wallTag || clsi == G_appleTag) clist.remove(clsi);
  });
...
function updScore(amt = 1) {
  score += amt;
  play_collectSFX();
  if (diff == 0 && score >= hi_ez) hi_ez = score;
  if (diff == 1 && score >= hi_med) hi_med = score;
  if (diff == 2 && score >= hi_hard) hi_hard = score;
  DisplayStuff();
  if (score <= -1) YouLose();
}
...
function DisplayStuff() {
  ...
  if (wallMode == 0) wallModeBtn.textContent = "Wall: Off";
  else if (wallMode == 1) wallModeBtn.textContent = "Wall: Ring";
  else if (wallMode == 2) wallModeBtn.textContent = "Wall: Random 5%";
  else if (wallMode == 3) wallModeBtn.textContent = "Wall: Random 10%";

  if (wallMode != 0)
    des_text = "Wall = lose (" + wallMode + "/" + (max_WallMode - 1) + ").";
